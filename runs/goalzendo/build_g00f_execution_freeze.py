...
from goalzendo_g00f.evaluator import create_preexecution_gate
...
DEFAULT_OUTPUT = "reproducibility/goalzendo/g00f-execution-freeze-20260811"
ARCHIVE_NAME = "g00f-execution-source.tar.gz"
BUNDLE_MANIFEST_NAME = "g00f-source-bundle-manifest.json"
...
def _source_manifest(repo: Path) -> dict[str, Any]:
    ...
    names = ("__init__.py", "cli.py", "evaluator.py", "freeze.py", "py.typed")
    files = {name: sha256_file(package / name) for name in names}
    body = {
        "schema": "goalzendo.g00f_additive_source_manifest",
        "schema_version": 1,
        "source_files": files,
...
def _bundle_paths(repo: Path) -> list[str]:
    ...
    paths.update(
        path.relative_to(repo).as_posix()
        for path in (repo / "src/goalzendo_g00f").rglob("*")
        if path.is_file() and "__pycache__" not in path.parts and not path.is_symlink()
    )
    for specification in CONFIG_SPECS.values():
...
def _freeze_payload(
    ...
    projected = sum(
        RUNS_PER_PANEL * float(specification["projected_h100_seconds_per_run"]) / 3_600
        for specification in CONFIG_SPECS.values()
    )
    ...
    body = {
        ...
        "additive_source": {
            "manifest_path": "src/goalzendo_g00f/source_manifest.json",
            "manifest_sha256": sha256_file(repo / "src/goalzendo_g00f/source_manifest.json"),
        ...
        "controller_files": {
            "bootstrap": _binding(repo, "runs/goalzendo/g00f_bundle_bootstrap.py"),
            "launcher": _binding(repo, "runs/goalzendo/run_g00f_frozen_4h100.sh"),
            "watchdog": _binding(repo, "runs/goalzendo/g00f_watchdog.py"),
        },
        "source_bundle": {
        ...
        "runtime": {
            ...
            "h100_hour_ceiling": H100_HOUR_CEILING,
            "projected_total_h100_hours": projected,
            "projected_h100_hours_per_worker": projected / WORKER_COUNT,
...
def build(repo: Path, output: Path) -> dict[str, Any]:
    ...
    source_manifest_path = repo / "src/goalzendo_g00f/source_manifest.json"
    ...
    readme = (
        b"# G00-F prospective execution freeze\n\n"
        ...
        b"fresh exact model materialization for both panels, and an externally "
        b"pinned Runpod provision receipt/raw API response.\n"
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(prog="build_g00f_execution_freeze")
    ...
    try:
        ...
        print(f"build_g00f_execution_freeze: error: {error}")
        return 2
