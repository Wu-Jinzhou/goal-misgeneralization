...
# Launch one native shard of the frozen Qwen3.5 evidence-geometry panel on the
# existing US-CA-2 research volume. The verified Qwen3.5 environment and model
# cache are reused read-only; there is no pilot or scientific-result gate.
...
if [[ ! "$SHARD_INDEX" =~ ^[0-7]$ ]]; then
  echo "SHARD_INDEX must be one of 0,1,2,3,4,5,6,7" >&2
  exit 64
fi
...
ARCHIVE="$JOB/input/goalzendo-qwen35-geometry-src.tgz"
EXTRACT_ROOT="$SHARD_ROOT/source"
SRC="$EXTRACT_ROOT/goalzendo-qwen35-geometry-src"
...
emit_status() {
  ...
  printf '{"schema":"goalzendo.qwen35_evidence_geometry_shard_status","schema_version":1,"job_id":"%s","shard_index":%s,"num_shards":8,"state":"%s","runner_rc":%s,"archive_sha256":"%s","config_sha256":"%s","implementation_fingerprint":"%s","started_at":"%s","finished_at":%s}\n' \
...
finish() {
  ...
  sync
  echo "GOALZENDO_GEOMETRY_SHARD_TERMINAL shard=$SHARD_INDEX state=$state rc=$rc finished_at=$finished"
  sleep infinity
}
trap finish EXIT
...
echo "GOALZENDO_GEOMETRY_SHARD_START shard=$SHARD_INDEX/8 started_at=$STARTED_AT"
...
observed_config=$(sha256sum configs/goalzendo/qwen35_evidence_geometry.yaml | awk '{print $1}')
...
expected_counts=(3 4 6 5 6 7 4 1)
...
config = load_config("configs/goalzendo/qwen35_evidence_geometry.yaml")
print(len(build_plan(config, shard_index=int(sys.argv[1]), num_shards=8)))
PY
)
...
echo "GOALZENDO_GEOMETRY_SHARD_VERIFIED shard=$SHARD_INDEX planned_runs=$observed_count"

"$VENV/bin/python" -m goalzendo.cli run \
  configs/goalzendo/qwen35_evidence_geometry.yaml \
...
print((datetime.now(timezone.utc) + timedelta(hours=30)).strftime("%Y-%m-%dT%H:%M:%SZ"))
PY
)
ENV_JSON=$(jq -cn \
  --arg job "$JOB_ID" \
  ...
  '{JOB_ID:$job,SHARD_INDEX:$shard,NUM_SHARDS:"8",ARCHIVE_SHA256:$archive,CONFIG_SHA256:$config,IMPLEMENTATION_FINGERPRINT:$implementation}')
printf -v TAG '%02d' "$SHARD_INDEX"
