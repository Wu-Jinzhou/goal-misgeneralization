...
from .freeze import (
    ...
    verify_qualification_supervisor_receipts,
)

ASSESSMENT_SCHEMA = "goalzendo.g00f_h200_assessment"
ASSESSMENT_SCHEMA_VERSION = 1
GATE_SCHEMA = "goalzendo.g00f_h200_gate"
GATE_SCHEMA_VERSION = 1
WORKER_RESULT_SCHEMA = "goalzendo.g00f_h200_worker_result_receipt"
...
def _verify_success_watchdog_lifecycle(
    *,
    execution_root: Path,
    verified: VerifiedFreeze,
    execution_uuid: str,
    budget_start_file_sha256: str,
    budget_started_monotonic_ns: int,
    worker_started_monotonic_values: Sequence[int],
    worker_completed_monotonic_values: Sequence[int],
) -> dict[str, Any]:
    """Replay the I/O-independent guardian and normal watchdog shutdown."""

    if not worker_started_monotonic_values or not worker_completed_monotonic_values:
        raise EvaluationError("watchdog replay requires complete worker timing evidence")
    deadline_monotonic_ns = budget_started_monotonic_ns + WALL_CEILING_SECONDS * 1_000_000_000
    paths = {
        "started": execution_root / "watchdog-started.json",
        "normal_stop": execution_root / "watchdog-normal-stop.json",
        "guardian_terminal": execution_root / "watchdog-guardian-terminal.json",
    }
    if any(
        not path.is_file()
        or path.is_symlink()
        ...
        raise EvaluationError("watchdog lifecycle receipt path, link count, or mode changed")
    receipts = {
        name: strict_json(path, f"G00-F watchdog {name.replace('_', '-')} receipt")
        for name, path in paths.items()
    }
    started = receipts["started"]
    stopped = receipts["normal_stop"]
    terminal = receipts["guardian_terminal"]
    bodies = {
        name: {key: value for key, value in receipt.items() if key != "receipt_digest"}
        for name, receipt in receipts.items()
    }
    common_keys = {
        "budget_start_file_sha256",
        "deadline_marker_lead_ns",
        ...
        "watchdog_receipt_margin_seconds",
    }
    shared_keys = common_keys - {"receipt_digest", "schema"}
    started_ns = started.get("started_monotonic_ns")
    stopped_ns = stopped.get("stopped_monotonic_ns")
    terminal_ns = terminal.get("completed_monotonic_ns")
    if (
        set(started) != common_keys | {"started_monotonic_ns"}
        or set(stopped) != common_keys | {"stopped_monotonic_ns"}
        or set(terminal)
        != common_keys
        | {
        ...
        or started.get("schema") != "goalzendo.g00f_h200_watchdog_started"
        or stopped.get("schema") != "goalzendo.g00f_h200_watchdog_normal_stop"
        or terminal.get("schema") != "goalzendo.g00f_h200_watchdog_guardian_terminal"
        or any(
            started.get(key) != stopped.get(key) or started.get(key) != terminal.get(key)
            for key in shared_keys
        )
        or started.get("schema_version") != 1
        or started.get("execution_uuid") != execution_uuid
        or started.get("freeze_file_sha256") != verified.file_sha256
        or started.get("freeze_digest") != verified.digest
        or started.get("budget_start_file_sha256") != budget_start_file_sha256
        or started.get("deadline_monotonic_ns") != deadline_monotonic_ns
        or isinstance(started.get("watchdog_pid"), bool)
        or not isinstance(started.get("watchdog_pid"), int)
        or int(started["watchdog_pid"]) <= 1
        or started.get("watchdog_process_group_id") != started.get("watchdog_pid")
        or isinstance(started.get("launcher_pid"), bool)
        or not isinstance(started.get("launcher_pid"), int)
        or int(started["launcher_pid"]) <= 1
        or started.get("launcher_process_group_id") != started.get("launcher_pid")
        or isinstance(started.get("guardian_pid"), bool)
        or not isinstance(started.get("guardian_pid"), int)
        or int(started["guardian_pid"]) <= 1
        or started.get("guardian_pid") in {started.get("watchdog_pid"), started.get("launcher_pid")}
        or started.get("guardian_protocol")
        != "ready_pipe_pidfd_launcher_liveness_and_monotonic_group_cutoff_v1"
        or started.get("term_grace_seconds") != 30
        or started.get("watchdog_receipt_margin_seconds") != 5
        or started.get("deadline_marker_lead_ns") != 2_000_000_000
        or isinstance(started_ns, bool)
        or not isinstance(started_ns, int)
        or isinstance(stopped_ns, bool)
        or not isinstance(stopped_ns, int)
        or isinstance(terminal_ns, bool)
        or not isinstance(terminal_ns, int)
        or started_ns < budget_started_monotonic_ns
        or started_ns > min(worker_started_monotonic_values)
        or stopped_ns < max(worker_completed_monotonic_values)
        or stopped_ns > deadline_monotonic_ns
        or terminal_ns < stopped_ns
        or terminal_ns > deadline_monotonic_ns
        or terminal.get("deadline_triggered") is not False
        or terminal.get("guardian_clean_stop") is not True
        or terminal.get("guardian_exit_code") != 0
        or started.get("outcome_metrics_read") is not False
        or started.get("predictions_read") is not False
        or started.get("g01_launch_authorized") is not False
        or any(
            receipt.get("receipt_digest") != semantic_digest(bodies[name])
            for name, receipt in receipts.items()
        )
    ):
        raise EvaluationError("watchdog lifecycle does not prove continuous 14-hour supervision")
    return {
        "started_file_sha256": sha256_file(paths["started"]),
        "normal_stop_file_sha256": sha256_file(paths["normal_stop"]),
        "guardian_terminal_file_sha256": sha256_file(paths["guardian_terminal"]),
        "deadline_monotonic_ns": deadline_monotonic_ns,
        "stopped_monotonic_ns": stopped_ns,
        "guardian_terminal_monotonic_ns": terminal_ns,
        "guardian_protocol": started["guardian_protocol"],
    }
    ...
    *,
    ...
    for worker_index in range(WORKER_COUNT):
        ...
        if (
            ...
            or receipt.get("selected_profile") != verified.selected_profile
        ...
        if (
            receipt.get("profile_qualification_cleanup")
            != verified_launch["ledger"]["profile_qualification_cleanup"]
        ):
            raise EvaluationError("worker result qualification cleanup binding mismatch")
    ...
    ceiling = float(verified.payload["runtime"]["h200_hour_ceiling"])
    if (
        total_hours > ceiling
    ...
    expected_root_names = {
        ...
        "h200-profile-qualification-cleanup.json",
        "h200-profile-qualification-evidence.json",
        "h200-profile-qualification-handoff.json",
        "h200-profile-qualification-producer-receipt.json",
        "h200-profile-qualification-transient-inventory.json",
        "h200-profile-qualification.json",
        "h200-profile-selection.json",
        "runpod-api-response.json",
        "runpod-create-response.json",
        "runpod-gpu-catalog-response.json",
        ...
        "storage-preflight-before-itt.json",
        "storage-preflight-before-qualification.json",
        "qualification-supervisor-started.json",
        "qualification-supervisor-terminal.json",
    ...
    profile_selection = verified.selection_receipt
    profile_qualification_cleanup = rows[0]["launch_receipt"]["ledger"].get("profile_qualification_cleanup")
    if (
        verified.selected_profile not in {"baseline", "tuned"}
        or not isinstance(profile_selection, Mapping)
        or profile_selection.get("execution_uuid") != execution_uuid
        or Path(str(profile_selection.get("path", ""))).resolve()
        != execution_root / "h200-profile-selection.json"
        or not isinstance(profile_qualification_cleanup, Mapping)
        or profile_qualification_cleanup.get("execution_uuid") != execution_uuid
        or Path(str(profile_qualification_cleanup.get("path", ""))).resolve()
        != execution_root / "h200-profile-qualification-cleanup.json"
    ):
        raise EvaluationError("worker evidence is not bound to one H200 profile selection and cleanup")
    qualification_handoff = profile_qualification_cleanup.get("qualification_handoff")
    if not isinstance(qualification_handoff, Mapping):
        raise EvaluationError("qualification cleanup omits its authenticated handoff")
    expected_controller_argv = qualification_handoff.get("controller_argv")
    if not isinstance(expected_controller_argv, list):
        raise EvaluationError("qualification handoff omits the controller argv")
    try:
        qualification_supervisor = verify_qualification_supervisor_receipts(
            verified=verified,
            execution_root=execution_root,
            expected_controller_argv=expected_controller_argv,
        )
    except FreezeError as error:
        raise EvaluationError("qualification supervisor failed exact replay") from error
    if qualification_handoff.get("qualification_supervisor") != qualification_supervisor:
        raise EvaluationError("qualification handoff changed its supervisor binding")
    watchdog = _verify_success_watchdog_lifecycle(
        execution_root=execution_root,
        verified=verified,
        execution_uuid=execution_uuid,
        budget_start_file_sha256=next(iter(budget_start_file_sha256s)),
        budget_started_monotonic_ns=next(iter(budget_started_monotonic_values)),
        worker_started_monotonic_values=worker_started_monotonic_values,
        worker_completed_monotonic_values=worker_completed_monotonic_values,
    )
    return {
        "workers": rows,
        ...
        "watchdog": watchdog,
        "total_h200_hours": total_hours,
        "h200_hour_ceiling": ceiling,
        "profile_selection": copy.deepcopy(dict(profile_selection)),
        "profile_qualification_cleanup": copy.deepcopy(dict(profile_qualification_cleanup)),
...
def _require_same_execution(
    ...
    if (
        ...
        or ledger.get("selected_profile") != runtime.get("selected_profile")
        or ledger.get("profile_selection") != runtime.get("profile_selection")
        or ledger.get("profile_qualification_cleanup") != runtime.get("profile_qualification_cleanup")
...
def _verify_attempt_receipt(
    *,
    ...
    if (
        payload.get("schema") != "goalzendo.g00f_h200_attempt_receipt"
    ...
    *,
    ...
    for frozen in verified.all_rows:
        ...
        if start is not None or state == "failed_before_start_receipt":
            ...
            if (
                lease.get("schema") != "goalzendo.g00f_h200_run_ownership_lease"
    ...
    if global_stop is not None:
        ...
        if (
            global_stop.get("schema") != "goalzendo.g00f_h200_global_execution_stop"
    ...
    if all_complete:
        ...
        if (
            unseal.get("schema") != "goalzendo.g00f_h200_panel_unseal_receipt"
...
def _verify_run_binding(
    *,
    ...
    if (
        ...
        or payload.get("selected_profile") != verified.selected_profile
        or payload.get("profile_selection") != verified.selection_receipt
    ...
    if (
        payload.get("profile_qualification_cleanup")
        != receipt_cache[launch_key]["ledger"]["profile_qualification_cleanup"]
    ):
        raise EvaluationError("per-run qualification cleanup differs from its launch receipt")
...
def _false_assessment(
    *,
    ...
    checks = {
        ...
        "h200_profile_selection": {
            "passed": verified.selected_profile in {"baseline", "tuned"}
            and isinstance(verified.selection_receipt, Mapping),
            "selected_profile": verified.selected_profile,
            "status": "verified" if verified.selection_receipt is not None else "not_selected",
        },
    ...
    body = {
        ...
        "profile_selection": (
            copy.deepcopy(dict(verified.selection_receipt))
            if isinstance(verified.selection_receipt, Mapping)
            else None
        ),
        "selected_profile": verified.selected_profile,
...
def derive_assessment(
    *,
    ...
    if verified.selected_profile not in {"baseline", "tuned"} or not isinstance(
        verified.selection_receipt, Mapping
    ):
        return _false_assessment(
            verified=verified,
            itt_rows=[],
            reason="authenticated_h200_profile_selection_missing",
        )
    ...
    try:
        ...
        checks["h200_profile_selection"] = {
            "passed": True,
            "selected_profile": verified.selected_profile,
            "receipt": copy.deepcopy(dict(verified.selection_receipt)),
        }
    ...
    rows = [
        {
            ...
            "state": "candidate_not_selected_not_run",
            "error_type": None,
            "attempt_count": 0,
            "intention_to_train": False,
        }
        for profile in sorted(verified.candidate_plans)
        for panel in verified.candidate_plans[profile].values()
        for row in panel
    ]
...
def verify_gate_artifact(
    ...
    *,
    ...
    if (
        ...
        or gate.get("profile_selection") != verified.selection_receipt
        or gate.get("selected_profile") != verified.selected_profile
        ...
        or assessment.get("profile_selection") != verified.selection_receipt
        or assessment.get("selected_profile") != verified.selected_profile
