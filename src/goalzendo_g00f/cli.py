...
def _repo(start: str | Path | None) -> Path:
    ...
    for candidate in (current, *current.parents):
        if (candidate / "pyproject.toml").is_file() and (candidate / "src" / "goalzendo_g00f").is_dir():
            return candidate
    raise FreezeError("could not locate the repository containing goalzendo_g00f")
...
def _actual_visible_gpu() -> dict[str, str]:
    ...
    if not gpu_name.startswith("NVIDIA H100") or "H100" not in torch_name:
        raise FreezeError("G00-F runtime is not backed by one NVIDIA H100")
    return {
...
def _create_launch_receipt(args: argparse.Namespace, verified: VerifiedFreeze) -> dict[str, Any]:
    ...
    if (
        ...
        or bundle.get("schema") != "goalzendo.g00f_extracted_source_bundle_receipt"
        ...
        or set(authenticated_runtime_files) != {"bootstrap", "launcher", "watchdog"}
    ...
    if (
        ...
        or "H100" not in str(devices[0].get("name", ""))
    ):
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="goalzendo-g00f")
    ...
    ledger.add_argument("--expected-provision-receipt-sha256", required=True)
    ledger.add_argument("--expected-pod-id", required=True)
    ...
    provision_create.add_argument("--pod-id", required=True)
    provision_create.add_argument("--created-at-utc", required=True)
    provision_create.add_argument("--terminate-after-utc", required=True)
    provision_create.add_argument("--secure-price-usd-per-gpu-hour", type=float, required=True)
    provision_create.add_argument("--stock-label", required=True)
    ...
    launch.add_argument("--expected-provision-receipt-sha256", required=True)
    launch.add_argument("--expected-pod-id", required=True)
    ...
    gate.add_argument("--expected-provision-receipt-sha256", required=True)
    gate.add_argument("--expected-pod-id", required=True)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "plan":
            rows = verified.all_rows if args.worker_index is None else verified.worker_rows(args.worker_index)
            for row in rows:
                _print(row)
            _print({"planned_runs": len(rows), "g01_launch_authorized": False})
            return 0
        ...
        if args.command == "create-provision-receipt":
            _print(
                create_runpod_provision_receipt(
                    ...
                    secure_price_usd_per_gpu_hour=args.secure_price_usd_per_gpu_hour,
        ...
        print(f"goalzendo-g00f: error: {error}", file=sys.stderr)
        return 2
    return 2
